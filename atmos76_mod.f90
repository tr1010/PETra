...
!============================================================================
  ...
  REAL*16,INTENT(OUT):: sigma  ! density/sea-level standard density              
  REAL*16,INTENT(OUT):: delta  ! pressure/sea-level standard pressure           
  REAL*16,INTENT(OUT):: theta  ! temperature/sea-level standard temperature
!============================================================================
!     L O C A L   C O N S T A N T S                                         |
!============================================================================
  REAL,PARAMETER:: REARTH = 6356.766               ! radius of the Earth (km)
...
!  theta=KineticTemperature(alt)/TZERO
  ...
  p=EvaluateCubic(Z(i),LOGP(i),DLOGPDZ(i), &
                      Z(i+1),LOGP(i+1),DLOGPDZ(i+1), alt)
  delta=p/LOG(PZERO)

  rho=EvaluateCubic(Z(i),LOGRHO(i),DLOGRHODZ(i), &
                      Z(i+1),LOGRHO(i+1),DLOGRHODZ(i+1), alt)
  sigma=rho/LOG(RHOZERO)
